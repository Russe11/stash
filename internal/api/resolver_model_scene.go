...
func (r *sceneResolver) Paths(ctx context.Context, obj *models.Scene) (*ScenePathsType, error) {
	...
	screenshotPath := builder.GetScreenshotURL(apiKey)
	previewPath := builder.GetStreamPreviewURL(apiKey)
	streamPath := builder.GetStreamURL(apiKey).String()
	webpPath := builder.GetStreamPreviewImageURL(apiKey)
	objHash := obj.GetHash(config.GetVideoFileNamingAlgorithm())
	vttPath := builder.GetSpriteVTTURL(objHash, apiKey)
	spritePath := builder.GetSpriteURL(objHash, apiKey)
	funscriptPath := builder.GetFunscriptURL(apiKey).String()
	captionBasePath := builder.GetCaptionURL(apiKey)
	interactiveHeatmap := builder.GetInteractiveHeatmapURL(apiKey)
